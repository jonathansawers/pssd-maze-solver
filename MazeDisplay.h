...
class MazeDisplay {
    ...
    public:
        ...
        void displayMazeWithColors(const std::vector<std::pair<int, int>>& path, const std::vector<std::pair<int, int>>& visited) const;
        
        // Helper methods for colored output
        void printColoredCellRow(int row, const std::vector<std::pair<int, int>>& path, const std::vector<std::pair<int, int>>& visited) const;
};
